...
import statefulMarkup from './server/handlers/handle-state-markup'
<% } -%>
...
<% if (locals.template === 'apollo') { -%>
routes.forEach(({ path }) => {
  coreRouter.get(path, ...shortCircuit(assets, statefulMarkup, render('default'), ssi))
  coreRouter.get(path, ctx => (ctx.body = ctx.template))
})
<% } -%>
